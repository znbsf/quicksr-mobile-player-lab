...
def draw_pixel_text(draw: ImageDraw.ImageDraw, origin: tuple[int, int], text: str, scale: int) -> None:
    glyphs = {
        ...
        for row_index, row in enumerate(rows):
            for column_index, bit in enumerate(row):
                if bit == "1":
                    ...
                    draw.rectangle((left, top, left + scale - 1, top + scale - 1), fill=(250, 250, 250))
        cursor += 6 * scale
...
def synthetic_lineart_subtitle_reference(width: int = 1280, height: int = 720) -> Image.Image:
    ...
    draw.rectangle(
        (width * 12 // 100, height * 80 // 100, width * 88 // 100, height * 96 // 100),
        fill=(8, 10, 16),
        outline=(250, 250, 250),
        width=max(1, width // 640),
    )
    text_scale = max(1, min(width // 180, height // 90))
    text = "ANIME SR TEST 24"
    text_width = len(text) * 6 * text_scale
    draw_pixel_text(draw, ((width - text_width) // 2, height * 84 // 100), text, text_scale)
    return image
...
def prepare_contract(output: Path, include_open_assets: bool) -> dict[str, Any]:
    ...
    sources: list[tuple[str, str, Image.Image, dict[str, Any]]] = [
        (
            "synthetic-lineart-subtitle-edge",
            "16:9",
            synthetic_lineart_subtitle_reference(),
            {"spdx": "LicenseRef-Project-Original-Fixture", "attribution": "Generated by project source"},
        )
    ]
    ...
    contract = {
        ...
        "limits": [
            "The synthetic fixture checks line, subtitle and edge behavior but is not representative anime quality evidence.",
