...
Status: candidate refresh plus bounded host and physical-device resident probes are complete for
IFRNet-S and RIFE v4.25-lite. Both candidates are stopped as RIFE v4.6 replacements. All results
remain offline-only. No file under `app/` changed, no model was added to the player, and no realtime,
sustained-thermal, representative-anime, or redistribution claim is made.

Follow-up: the RIFE v4.25-lite modern-runtime three-level physical-device matrix is complete; see
`ANIME_VFI_RIFE_V425_LITE_PROBE.md`.
...
Do not replace RIFE v4.6 with RIFE v4.25-lite either. Its exact modern runtime avoids the 2022
executable's `MemoryData` blocker and is compatible with the observed SM8550/Adreno 740 device, but
only `256x144` is faster. The `160x90` and `320x180` medians are 77.2% and 56.0% slower, sampled PSS
is slightly higher at every level, and synthetic quality is worse at two levels and better at one.
The mixed result has no defensible player margin, so the candidate is stopped rather than integrated.
...
| --- | --- | --- | --- | --- |
| RIFE v4.25-lite ncnn | [Practical-RIFE at `bbfd2ea`](https://github.com/hzwer/Practical-RIFE/tree/bbfd2ea90910789a860ea3e2b32a240cd577b75e) says v4.25 improves anime scenes and defines `lite` as lower compute; [TNTwise ncnn port at `13338e3`](https://github.com/TNTwise/rife-ncnn-vulkan/tree/13338e38debe2e400b3eeecf6792312d01a692f9) contains the converted model | Both repositories are MIT. Practical-RIFE says linked trained models share its MIT license, but transformed-weight redistribution is still blocked here pending a human provenance review | 11,312,473 checked-out bytes; custom `rife.Warp` plus `MemoryData`, conv/deconv, interpolation, pixel shuffle, elementwise and split/concat operators; project-built Android arm64 CLI only | Modern host/device gates completed; inconsistent speed/quality, stopped |
