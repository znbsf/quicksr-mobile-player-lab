...
| Anime VFI 离线探针 | HOST + DEVICE RESIDENT MATRIX（单设备、有界） | prefilter 仍只接受 distinct drawing；RIFE v4.6 在 Adreno 740 的单进程 5 档矩阵均完成。160x90 稳态中位 30.335 ms 但 max 33.343 ms，320x180 中位 36.106 ms 但 max 42.868 ms；采样 PSS 峰值 186,107-201,912 kB | 未进入播放器或 APK；解码/SR/合成/A-V sync 等成本未计，低分辨率也无可靠实时余量，仍为 offline-only；权重再分发、代表性时序质量、LPIPS/warp/flicker、人工审核和长时热稳均未证明 |
