...
| Anime VFI 离线探针 | HOST + DEVICE RESIDENT MATRIX（单设备、有界） | manifest、本地 7+6 文件、实际 prefilter 决策和设备端输入 hash 已 fail-closed 绑定；RIFE v4.6 五档最终重跑均完成。160x90 稳态中位/max 为 30.260/32.008 ms，320x180 为 35.715/37.806 ms；采样 PSS 峰值 185,929-187,337 kB | 未进入播放器或 APK；两档纯模型虽分别守住 30/24 fps，但最大余量仅约 1.325/3.860 ms，解码/SR/合成/A-V sync 等成本未计，仍为 offline-only；权重再分发、代表性时序质量、LPIPS/warp/flicker、人工审核和长时热稳均未证明 |
