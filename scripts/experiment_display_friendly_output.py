#!/usr/bin/env python3
"""Build and validate a display-friendly output wrapper around a frozen QuickSR graph.

This experiment keeps every learned weight and convolution unchanged. It replaces the public
float32 NCHW graph output with uint8 NHWC RGB so a runtime can avoid exporting a 23.73 MiB
1920x1080 float tensor and CPU-packing it into interleaved display pixels.

The generated ONNX file is an ignored build artifact. This script is a host correctness and data
volume probe only; it does not establish that QNN places the appended operators on HTP.
"""
...
def build_display_graph(source_path: Path, output_path: Path) -> tuple[str, str, list[int], list[int]]:
...
def main() -> int:
    ...
    input_name, _, input_shape, output_shape = build_display_graph(args.source, args.output)
    ...
    expected = np.transpose(
        np.clip(source_output, 0.0, 1.0) * np.float32(255.0) + np.float32(0.5),
        (0, 2, 3, 1),
    )
    ...
    result = {
        ...
        "display_rgb8_output_bytes": display_bytes,
