#!/usr/bin/env python3
"""Derive and validate a fixed 512x288 DCR QuickSRNetSmall x2 model."""
...
MODEL_PATH = DERIVED_ROOT / "quicksrnet-small-2x-fixed512x288-dcr.onnx"
MANIFEST_PATH = DERIVED_ROOT / "derivation-manifest-fixed512x288.json"
...
def derive_model(canonical: onnx.ModelProto) -> onnx.ModelProto:
    ...
    model.graph.name = "quicksrnet_small_2x_fixed512x288_dcr_full"
    model.producer_name = "quicksrnet-fixed512x288-deriver"
    model.producer_version = "1"
    model.model_version = 1
    model.doc_string = (
        "Deterministic fixed 512x288 DCR derivative of the frozen "
        "QuickSRNetSmall x2 ONNX."
    )
    helper.set_model_props(
        model,
        {
            "derivation.variant": "fixed512x288-dcr-full",
...
def validate_equivalence(
    ...
    try:
        ...
        for case_id, input_tensor in (
            ("rgb-gradient-512x288", gradient_nchw()),
            ("seeded-random-512x288", seeded_random_nchw()),
        ):
            ...
            if not comparison["exact_float32_bytes"]:
                raise base.DerivationError(
                    f"fixed512x288 output is not byte-exact for {case_id}: {comparison}"
                )
            results.append(
                {
                    "id": case_id,
...
def generate() -> dict[str, Any]:
    ...
    manifest = {
        "schema_version": 1,
        "document_kind": "deterministic-onnx-fixed512x288-derivation-manifest",
        "status": "pass",
        "source": {
        ...
        "artifact": base._model_record(
            ...
            [
                ...
                "freeze input shape to [1,3,288,512]",
                "freeze output shape to [1,3,576,1024]",
            ],
        ),
