#!/usr/bin/env python3
"""Derive and validate a fixed 256x144 DCR QuickSRNetSmall x2 model."""
...
MODEL_PATH = DERIVED_ROOT / "quicksrnet-small-2x-fixed256x144-dcr.onnx"
MANIFEST_PATH = DERIVED_ROOT / "derivation-manifest-fixed256x144.json"
...
def derive_model(canonical: onnx.ModelProto) -> onnx.ModelProto:
    ...
    model.graph.name = "quicksrnet_small_2x_fixed256x144_dcr_full"
    model.producer_name = "quicksrnet-fixed256x144-deriver"
    model.producer_version = "1"
    model.model_version = 1
    model.doc_string = (
        "Deterministic fixed 256x144 DCR derivative of the frozen "
        "QuickSRNetSmall x2 ONNX."
    )
    helper.set_model_props(
        model,
        {
            "derivation.variant": "fixed256x144-dcr-full",
...
def validate_equivalence(
    ...
    try:
        ...
        for case_id, input_tensor in (
            ("rgb-gradient-256x144", gradient_nchw()),
            ("seeded-random-256x144", seeded_random_nchw()),
        ):
            ...
            if not comparison["exact_float32_bytes"]:
                raise base.DerivationError(
                    f"fixed256x144 output is not byte-exact for {case_id}: {comparison}"
                )
            results.append(
                {
                    "id": case_id,
...
def generate() -> dict[str, Any]:
    ...
    manifest = {
        "schema_version": 1,
        "document_kind": "deterministic-onnx-fixed256x144-derivation-manifest",
        "status": "pass",
        "source": {
        ...
        "artifact": base._model_record(
            ...
            [
                ...
                "freeze input shape to [1,3,144,256]",
                "freeze output shape to [1,3,288,512]",
            ],
        ),
