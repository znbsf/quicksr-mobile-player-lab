#!/usr/bin/env python3
"""Derive and validate a fixed-512 DCR QuickSRNetSmall x2 model.

The canonical, fixed-64, and fixed-256 artifacts are immutable inputs. This
script writes only the fixed-512 model and its separate manifest beside itself.
"""
...
MODEL_PATH = DERIVED_ROOT / "quicksrnet-small-2x-fixed512-dcr.onnx"
MANIFEST_PATH = DERIVED_ROOT / "derivation-manifest-fixed512.json"

EXPECTED_FIXED256_MODEL_SHA256 = (
    "f791fdc975862b2556eca8113cfb9139c0b3c32303c86ce85497298024ce89be"
)
EXPECTED_FIXED256_MANIFEST_SHA256 = (
    "c7e20ebf21f87b08368f0c6fbaf8fa6a0e0b6287ce6ff816a9917bf7a7cd117f"
)
...
def derive_model(canonical: onnx.ModelProto) -> onnx.ModelProto:
    ...
    model.graph.name = "quicksrnet_small_2x_fixed512_dcr_full"
    model.producer_name = "quicksrnet-fixed512-deriver"
    model.producer_version = "1"
    model.model_version = 1
    model.doc_string = (
        "Deterministic fixed512 DCR derivative of the frozen "
        "QuickSRNetSmall x2 ONNX."
    )
    helper.set_model_props(
        model,
        {
            "derivation.variant": "fixed512-dcr-full",
            ...
            "derivation.base_fixed256_sha256": EXPECTED_FIXED256_MODEL_SHA256,
        },
    )
...
def validate_equivalence(
    ...
    try:
        cases = (
            ("rgb-gradient-512", gradient_nchw()),
            ("seeded-random-512", seeded_random_nchw()),
        )
        ...
        for case_id, input_tensor in cases:
            ...
            if not comparison["exact_float32_bytes"]:
                raise base.DerivationError(
                    f"fixed512 output is not byte-exact for {case_id}: {comparison}"
                )
            results.append(
                {
                    "id": case_id,
...
def generate() -> dict[str, Any]:
    ...
    fixed64_before = {
        "core": base.sha256_bytes(
            _require_unchanged(
                ...
                fixed256.EXPECTED_FIXED64_CORE_SHA256,
                "fixed64 core ONNX",
            )
        ),
        ...
        "manifest": base.sha256_bytes(
            _require_unchanged(
                base.MANIFEST_PATH,
                fixed256.EXPECTED_FIXED64_MANIFEST_SHA256,
                "fixed64 derivation manifest",
            )
        ),
    }
    fixed256_before = {
        "model": base.sha256_bytes(
            _require_unchanged(
                fixed256.MODEL_PATH,
                EXPECTED_FIXED256_MODEL_SHA256,
                "fixed256 DCR ONNX",
        ...
        "manifest": base.sha256_bytes(
            _require_unchanged(
                fixed256.MANIFEST_PATH,
                EXPECTED_FIXED256_MANIFEST_SHA256,
                "fixed256 derivation manifest",
            )
        ),
    }
    ...
    fixed256_after = {
        "model": base.sha256_bytes(fixed256.MODEL_PATH.read_bytes()),
        "manifest": base.sha256_bytes(fixed256.MANIFEST_PATH.read_bytes()),
    }
    if fixed64_after != fixed64_before or fixed256_after != fixed256_before:
        raise base.DerivationError(
            "earlier artifacts changed during fixed512 derivation: "
            f"fixed64 before={fixed64_before}, after={fixed64_after}; "
            f"fixed256 before={fixed256_before}, after={fixed256_after}"
        )

    script_bytes = Path(__file__).read_bytes()
    base_script_bytes = Path(base.__file__).read_bytes()
    fixed256_script_bytes = Path(fixed256.__file__).read_bytes()
    manifest = {
        "schema_version": 1,
        "document_kind": "deterministic-onnx-fixed512-derivation-manifest",
        "status": "pass",
        "source": {
        ...
        "fixed256_inputs_unchanged": fixed256_after,
        "derivation_tool": {
            "path": _lab_relative(Path(__file__)),
            "sha256": base.sha256_bytes(script_bytes),
            "fixed256_tool_path": _lab_relative(Path(fixed256.__file__)),
            "fixed256_tool_sha256": base.sha256_bytes(fixed256_script_bytes),
        ...
        "artifact": base._model_record(
            ...
            [
                ...
                "freeze input shape to [1,3,512,512]",
                "freeze output shape to [1,3,1024,1024]",
            ],
        ),
        "pc_ort_validation": validation,
        "canonical_was_written": False,
        "fixed64_was_written": False,
        "fixed256_was_written": False,
    }
    manifest_bytes = (
        ...
        json.dumps(
            {
                ...
                "fixed256_inputs_unchanged": manifest[
                    "fixed256_inputs_unchanged"
                ],
            },
            indent=2,
            sort_keys=True,
