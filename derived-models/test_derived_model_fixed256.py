...
import derive_quicksrnet_fixed256 as derive
...
class Fixed256DerivationTests(unittest.TestCase):
    @classmethod
    ...
    def test_fixed64_inputs_are_unchanged(self) -> None:
        self.assertEqual(
            file_sha256(derive.base.CORE_MODEL_PATH),
            derive.EXPECTED_FIXED64_CORE_SHA256,
        )
        self.assertEqual(
            file_sha256(derive.base.DCR_MODEL_PATH),
            derive.EXPECTED_FIXED64_DCR_SHA256,
        )
        self.assertEqual(
            file_sha256(derive.base.MANIFEST_PATH),
            derive.EXPECTED_FIXED64_MANIFEST_SHA256,
        )

    def test_graph_is_fixed256_dcr(self) -> None:
        onnx.checker.check_model(self.model, full_check=True)
        self.assertEqual(value_shape(self.model.graph.input[0]), [1, 3, 256, 256])
        self.assertEqual(value_shape(self.model.graph.output[0]), [1, 3, 512, 512])
        self.assertEqual(
    ...
    def test_manifest_records_byte_exact_pc_validation(self) -> None:
        ...
        self.assertEqual(
            ...
            ["rgb-gradient-256", "seeded-random-256"],
        )
